...
// 현재 서비스 타입 저장 (stock 또는 crypto)
export function setCurrentService(service) {
  if (typeof window !== 'undefined') {
    localStorage.setItem('quanter_current_service', service)
  }
}

export function getCurrentService() {
  if (typeof window !== 'undefined') {
    return localStorage.getItem('quanter_current_service') || 'stock'
  }
  return 'stock'
}
...
export function StockNavBar({ active }) {
  ...
  const items = [
    ...
    { id: "pricing", label: "구매", path: "/pricing",
      icon: (c) => <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="7" stroke={c} strokeWidth="1.5"/><path d="M10 13v-6M8 7h4" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg> },
    ...
    { id: "switch",  label: "코인", path: "/crypto",
      icon: (c) => <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M13 16l4-4m0 0l-4-4m4 4H3M7 4l-4 4m0 0l4 4" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
  ]
...
export function CryptoNavBar({ active }) {
  ...
  const items = [
    ...
    { id: "account", label: "계정", path: "/account",
      icon: (c) => <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="7" r="3" stroke={c} strokeWidth="1.5"/><path d="M4 17c0-3.31 2.69-6 6-6s6 2.69 6 6" stroke={c} strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { id: "switch", label: "주식", path: "/stock",
      icon: (c) => <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M7 16l-4-4m0 0l4-4m-4 4h14M13 4l4 4m0 0l-4 4" stroke={c} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> },
  ]
  return <NavBarBase items={items} active={active} navigate={navigate} />
}

// 동적 NavBar - localStorage에서 현재 서비스 타입 읽어서 표시
export function DynamicNavBar({ active }) {
  const [currentService, setCurrentServiceState] = useState('stock')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const newService = getCurrentService()
      setCurrentServiceState(prev => prev !== newService ? newService : prev)
    }, 200)
  ...
  if (currentService === 'crypto') {
    return <CryptoNavBar active={active} />
  }
  return <StockNavBar active={active} />
}
